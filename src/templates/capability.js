...
const capabilityTemplate = ( {data: {capabilities} } ) => {
  return (
    <Layout noHero>
      <SEO
        title={`Hire ${capabilities.name} developers`}
        keywords={['weknowinc', capabilities.name]}
        description={`Hire ${capabilities.name} developers`}
        url={`${process.env.PROJECT_URL}/hire-${capabilities.id}-developers`}
      />
      <div className="container md:flex flex-wrap mx-auto py-2 lg:py-4 leading-normal px-3 text-2xl font-serif text-darker font-thin">
        ...
        <div className="md:w-1/2 pr-3">
          <div  className={`container mx-auto mb-5 text-2xl text-darker text-left font-serif`} >
            <h2 className="text-4xl md:text-5xl text-secondary mb-3 leading-tight">{`Hire weKnow's talented ${capabilities.name} Developers`}</h2>
            <p>weKnow is a development agency with highly skilled {capabilities.name} developers, programmers, engineers and coders from around the world. Providing top Onshore / Nearshore {capabilities.name} talent to help companies scale their development teams.</p>
          </div>
        ...
        <div className="container mx-auto">
          ...
          <div className="md:flex my-5 flex-wrap p-4 lg:p-0">
            <div className="w-full lg:w-1/2 lg:pr-3">
              <img src="https://weknowinc.com/modules/custom/keeptrack/images/taas/taas-1.jpg" alt="" className="w-full"/>
            </div>
            <div className="lg:w-1/2 lg:pl-3 flex flex-wrap items-center justify-center mt-4 md:mt-0">
              <FaChartBar className="w-2/12 md:w-1/12 h-4 text-4xl text-info pr-2"/>
              ...
              <FaFileAlt className="w-2/12 md:w-1/12 h-4 text-4xl text-info pr-2"/>
              <div className="w-10/12 md:w-11/12">
              ...
              <FaUser className="w-2/12 md:w-1/12 h-4 text-4xl text-info pr-2"/>
              <div className="w-10/12 md:w-11/12">
              ...
              <FaTrophy className="w-2/12 md:w-1/12 h-4 text-4xl text-info pr-2"/>
              <div className="w-10/12 md:w-11/12">
      ...
      <div className="flex flex-wrap container mx-auto flex-wrap my-4 px-4 md:px-0 justify-center items-center">
        {capabilities.skills && capabilities.skills.map((skill)=>(
          <div key={skill.id} className="w-full md:w-1/3  text-darker text-3xl text-center px-2">
